...
function gdrcd_bbcoder($str){
    ...
    $replace = array(
      ...
      '<li>$1</li>',
      '<img src="http://riverstonegdr.altervista.org/divisorio.png"></img>',
      ...
      '<img src="$1" class="img_generale">',
      '<meta http-equiv="Refresh" content="5;url=$1">',
      '<a href="$1">$2</a>',
      '<iframe width="450" height="280" src="$1" frameborder="0" allowfullscreen></iframe>',
 ...
 */
function gdrcd_chatcolor($str, $tipo)
{
    switch ($tipo) {
        case 'P':
            $search = array('#\&lt;(.+?)\&gt;#is',    '#\[(.+?)\]#is',   );
            $replace = array(	'<span class="color2">&lt;$1&gt;</span>','<span class="color2">&lt;$1&gt;</span>', );

            break;
        case 'M':
            $search = array('#\&lt;(.+?)\&gt;#is',    '#\[(.+?)\]#is',   );
            $replace = array(	'<span class="colorMaster">&lt;$1&gt;</span>','<span class="colorMaster">&lt;$1&gt;</span>', );
            break;
        default:
            $search = array('#\&lt;(.+?)\&gt;#is',    '#\[(.+?)\]#is',   );
            $replace = array(	'<span class="color2">&lt;$1&gt;</span>','<span class="color2">&lt;$1&gt;</span>', );
...
{
	$search = $user;
    $replace = '<span class="chat_me" style="font-variant:small-caps; color:goldenrod; text-decoration:underline; font-weight: bold; font-size:16px;">'.$search.'</span>';
...
{
	$search = $user;
    $replace = '<span style="font-variant:small-caps; color:#6E1D1D; text-decoration: underline; font-weight: bold; font-size:16px;">'.$search.'</span>';
    return str_ireplace($search, $replace, $str);
}
/**
