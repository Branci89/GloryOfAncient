 ...
 * @param string $_str : la stringa con i bbcode da tradurre, dovrebbe già essere stata filtrata per l'output su pagina web
 * @return $str con i tag bbcode tradotti in html
 * @author Blancks
 */
function gdrcd_bbcoder($_str) {
    global $MESSAGE;
    $str = gdrcd_close_tags('quote', $_str);
...
/**
 * @author Brancix (Discord: Brancix#8773)
 * Questa funzione imposta il pre-set dei tratti caratteriali definiti dalla cultura scelta
 * durante la fase di iscrizione
 * @param string $nomePg il nome del personaggio a cui assegnare i tratti
 * @param int $idCultura l'id numerico della cultura scelta
 * @param int $puntiIniziali sono il numero di punti da distribuire ai tratti caratteriali dopo l'iscrizione
 * @return void 
 */
function setta_tratti_cultura($nomePg, $idCultura, $puntiIniziali) {
    ...
    //costruisco la query 
    $query = "INSERT INTO tratti (personaggio, ";
    $field = "";
    $valori = "";
    for ($i = 1; $i <= $countTratti; $i++) {
        $field = $field . " tratto" . $i . ",";
    }
    $field = $field . " da_spendere)";
    $query = $query . $field;
    $query = $query . " VALUES('" . $nomePg . "',";
    //ora andiamo con i valori
    for ($i = 1; $i <= $countTratti; $i++) {
        $valori = $valori . "" . $cultura['tratto' . $i] . ",";
    }
    //da_spendere
    $valori = $valori . "" . gdrcd_filter('num', $puntiIniziali) . ")";
