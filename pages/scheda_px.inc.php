...
<div class="page_title">
   ...
   if ($_POST['op']=="assegna"){
	  if((is_numeric($_POST['px'])===TRUE)&&($_SESSION['permessi']>=GAMEMASTER)){
		...
		gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento ,descrizione_evento) VALUES ('".gdrcd_filter('in',$_REQUEST['pg'])."', '".$_SESSION['login']."', NOW(), ".PX.", '(".gdrcd_filter('num',$_POST['px']).' px) '.gdrcd_filter('in',$_POST['causale'])."')");
...
$query="SELECT  descrizione_evento, autore, data_evento FROM log WHERE nome_interessato = '".gdrcd_filter('in',$_REQUEST['pg'])."' AND codice_evento = ".PX." ORDER BY data_evento DESC LIMIT ".$num_logs."";
$result=gdrcd_query($query, 'result');
