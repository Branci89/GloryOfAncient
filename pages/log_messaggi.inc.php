...
<?php /*Form di scelta del log (visualizzazione di base)*/
   if ((isset($_POST['op'])===FALSE)&&(isset($_REQUEST['op'])===FALSE)) { ?>
    ...
    <form action="main.php?page=log_messaggi"
	      method="post">
		  <?php
...
<?php //*Elenco log*/
	...
	$record_globale=gdrcd_query("SELECT COUNT(*) FROM messaggi WHERE mittente = '".gdrcd_filter('in',$_REQUEST['pg'])."'");
	$totaleresults=$record_globale['COUNT(*)'];
	//Lettura record
	$result=gdrcd_query("SELECT destinatario, spedito, testo FROM messaggi WHERE mittente = '".gdrcd_filter('in',$_REQUEST['pg'])."' ORDER BY spedito DESC LIMIT ".$pagebegin.", ".$pageend."", 'result'); 
