...
while ($jobs=gdrcd_query($result, 'fetch')){
	$jobsn++;
	if($disoccupato==1){$disoccupato=0;/*Il pg e' affiliato ad una gilda*/}
...
if(isset($_POST['op'])===FALSE){  
		...
		<table>
		<tr>
		<td class="casella_titolo" colspan="2"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['job']['job']); ?></div></td>
		<td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['job']['pay']); ?></div></td>
		<td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['job']['controls']); ?></div></td>
		</tr>
	    <?php while($row=gdrcd_query($result, 'fetch')){ ?>
	   
		
		    <!--table class="lavori_box" border="1"--><tr>
			  <td class="casella_elemento_img"><div class="icone_elenco"><img class="" src="themes/<?php echo $PARAMETERS['themes']['current_theme'];?>/imgs/guilds/<?php echo $row['immagine']; ?>"/></div></td>
		      <td class="casella_elemento"><div class="elementi_elenco"><?php echo $row['nome_ruolo']; ?></div></td>
			  <td class="casella_elemento"><div class="elementi_elenco"><?php echo $row['stipendio'].' '.gdrcd_filter('out',$PARAMETERS['names']['currency']['plur']); ?></div></td>
		      <td  class="casella_controlli"> 
			    <div class="controllo_elenco">
				<form method="post"
			          action="main.php?page=servizi_lavoro">
			    <?php if ($ultimolavoro <= strftime("%Y-%m-%d")){ 
				  ...
				  <?php  } else if ($jobsn < $PARAMETERS['settings']['guilds_limit']) { ?>
				  <input type="submit"
						 value="<?php echo gdrcd_filter('out',$MESSAGE['interface']['job']['submit']['pick']); ?>"/>
				  <input type="hidden"
				         name="op"
						 value="pick"/>
				  <?php } else {echo '&nbsp;';}?>
				  <input type="hidden"
				         name="nome_lavoro"
						 value="<?php echo gdrcd_filter('out',$row['nome_ruolo']); ?>"/>
				  <input type="hidden"
				         name="id_record"
						 value="<?php echo $row['id_ruolo']; ?>"/>
				<?php } else if ($lavoro==$row['id_ruolo']) {
						   $ultimolavoroexp=explode("-",$ultimolavoro); 
                           echo gdrcd_filter('out',$MESSAGE['interface']['job']['extent'])." ".$ultimolavoroexp[2]."-".$ultimolavoroexp[1]."-".$ultimolavoroexp[0];
					  } else { echo '&nbsp;';} //else if?>
				</form>
			    </div>
			  </td></tr>
