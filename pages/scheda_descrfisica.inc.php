...
</div><!-- Background, descrfisica, robe varie -->

<div class="background"><!-- Background, descrfisica, robe varie -->

  <div class="titolo_box">
     <?php echo gdrcd_filter('out',$MESSAGE['interface']['sheet']['box_title']['descrfisica']); ?>
  </div>
     ...
     <?php 
		...
		*/	
		if ($PARAMETERS['mode']['user_bbcode'] == 'ON')
		{
			...
			{
				echo bbdecoder(gdrcd_html_filter($record['descrfisica']), true);
			
			}elseif ($PARAMETERS['settings']['user_bbcode']['type'] == 'bbd')
			{
				echo bbdecoder(gdrcd_filter('out',$record['descrfisica']), true);
			
			}else
			{
				echo gdrcd_bbcoder(gdrcd_filter('out',$record['descrfisica']));
			}
		
		}else
		{
			echo gdrcd_html_filter($record['descrfisica']);
		}
	?>
