...
//echo $info['invitati']; echo gdrcd_capital_letter($_SESSION['login']);
            ...
            <div class="panels_box"><div class="form_chat">
                    ...
                    <div class="form_row">
                        <form action="pages/chat.inc.php?ref=10&chat=yes" method="post" target="chat_frame" id="chat_form_messages">
                            ...
                            <div class="casella_chat">
                                ...
                                <!--NotePad-->
                                (<a href="#" onClick="javascript:window.open('/pages/notepad.php', 'NotePad', 'width=500,height=500')">NOTEPAD</a>)
                                <!--NotePad-->
                            </div>
                    ...
                    <?php if (($PARAMETERS['mode']['skillsystem'] == 'ON') || ($PARAMETERS['mode']['dices'] == 'ON')) { ?>
                        <div class="form_row"  style="margin-left:-12px">
                            <form action="pages/chat.inc.php?ref=30&chat=yes" method="post" target="chat_frame"  id="chat_form_actions">
                                ...
                                <?php if ($PARAMETERS['mode']['dices'] == 'ON') { ?>
                                    ...
                                    <div class="casella_chat">
                                        <select name="dice" id="dice">
                                            <option value="no_dice">Seleziona</option>
                                            <option value="100">Tenta la sorte</option>
                                            <?php
                                ...
                                <?php if ($PARAMETERS['mode']['skillsystem'] == 'ON') { ?>
                                    ...
                                    <div class="casella_chat">
                                        <?php $result = gdrcd_query("SELECT clgpersonaggiooggetto.id_oggetto, oggetto.nome, clgpersonaggiooggetto.cariche FROM clgpersonaggiooggetto JOIN oggetto ON clgpersonaggiooggetto.id_oggetto = oggetto.id_oggetto WHERE oggetto.car != -1 and clgpersonaggiooggetto.nome = '" . $_SESSION['login'] . "' AND posizione > 0 AND oggetto.tipo=13 ORDER BY oggetto.nome", 'result'); ?>
                                        ...
                                        <br/>
                                        <span class="casella_info"><?php echo gdrcd_filter('out', $MESSAGE['chat']['commands']['item']); ?></span>
                                    </div>
                                    
                                <div class="casella_chat">
                                        <?php $result = gdrcd_query("SELECT clgpersonaggiotalento.id_talento, talento.nome, clgpersonaggiotalento.numero FROM clgpersonaggiotalento JOIN talento ON clgpersonaggiotalento.id_talento = talento.id_talento WHERE talento.car != -1 and clgpersonaggiotalento.nome = '" . $_SESSION['login'] . "'  ORDER BY talento.nome", 'result'); ?>
                                        <select name="id_talent" id="id_talent" >
                                            <option value="no_talent"></option>
                                            <?php while ($row = gdrcd_query($result, 'fetch')) { ?>
                                                <option value="<?php echo $row['id_talento'] . '-' . $row['numero'] . '-' . gdrcd_filter('out', $row['nome']); ?>">
                                                    <?php echo $row['nome']  ; ?>
                                                </option>
                                                <?php
                                            }//while 
                                        ...
                                        <br/>
                                        <span class="casella_info">Usa Dono</span>
                                    </div>
                            ...
                            <!--SalvaChat-->
                                <?php if ($PARAMETERS['mode']['chatsave'] == 'ON') { ?>
                                    ...
                                    <a title="Salva Giocata" href="javascript:void(0);" onClick="window.open('chat_save.proc.php', 'Log', 'width=1200,height=500,toolbar=no');">
                                        <button class="btn-dark-moon btn-fab">Salva Chat</i></button>				 			
                                    </a>
                                <?php } ?>
                        </div>
                    <?php } ?>
