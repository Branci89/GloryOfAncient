<div class="pagina_messages_center">
    ...
    <div class="page_body">
        ...
        <?php
        /* Visualizzazione completa di un messaggio */
        ...
        <?php echo gdrcd_bbcoder(gdrcd_filter('out', $record['testo'])); ?>
                    </div>
    ...
    <?php echo gdrcd_filter('out', $PARAMETERS['names']['private_message']['sing'] . $MESSAGE['interface']['messages']['erased']); ?>
        ...
        <?php
        /* Elenco messaggi (visualizzazione di base della pagina) */
        if ((isset($_REQUEST['op']) === FALSE) && (isset($_REQUEST['newmessage']) === FALSE)) {
            ...
            <center><table><tr style="background:transparent;"><td align=center><div class="link_bottoni">
    ...
    <?php if ($numresults > 0) { ?>
        ...
        <?php while ($row = gdrcd_query($result, 'fetch')) { ?>
                            <tr>
                                <td  class="check"><input type="checkbox" class="message_check" value="<?php echo (int) $row['id'] ?>" /></td>
                                <td>
            ...
            <?php
            if ((($row['destinatario'] == 'all') && ($_SESSION['permessi'] >= MODERATOR)) ||
