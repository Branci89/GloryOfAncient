...
if (gdrcd_query($result, 'num_rows')==0){echo '<div class="error">'.gdrcd_filter('out',$MESSAGE['error']['unknown_character_sheet']).'</div>';}
   ...
   if ((gdrcd_filter('get',$_POST['op'])=="cedi") && (($_SESSION['login']==$_REQUEST['pg'])||($_SESSION['permessi']>=GAMEMASTER))){ /*Rimuovo un oggetto*/
	  ...
	  if(gdrcd_query($result_min, 'num_rows')>0)
	  {
         ...
         echo '<div class="warning">'.gdrcd_filter('out',$MESSAGE['warning']['done']).'</div>';
	  } else { echo '<div class="warning">'.gdrcd_filter('out',$MESSAGE['warning']['cant_do']).'</div>';}
   }
  ...
  <?php 
		...
		*/	
		if ($PARAMETERS['mode']['user_bbcode'] == 'ON')
		{
			if ($PARAMETERS['settings']['user_bbcode']['type'] == 'bbd' && $PARAMETERS['settings']['bbd']['free_html'] == 'ON')
			{
				echo bbdecoder(gdrcd_html_filter('out',$MESSAGE['interface']['sheet']['items']['list']['notes'].": ".$record['commento']), true);
			
			}elseif ($PARAMETERS['settings']['user_bbcode']['type'] == 'bbd')
			{
				echo bbdecoder(gdrcd_filter('out',$MESSAGE['interface']['sheet']['items']['list']['notes'].": ".$record['commento']), true);
			
			}else
			{
				echo gdrcd_bbcoder(gdrcd_filter('out',$MESSAGE['interface']['sheet']['items']['list']['notes'].": ".$record['commento']));
			}
		
		}else
		{
			echo gdrcd_html_filter('out',$MESSAGE['interface']['sheet']['items']['list']['notes'].": ".$record['commento']);
		}
   ...
   <div class="form_gioco">
	 ...
	 <form action="popup.php?page=scheda_equip"
        ...
        <input type="hidden" 
			   ...
			   name="pg" />
		<input type="hidden" 
		       value="<?php echo gdrcd_filter('out',$record['nome_oggetto']); ?>"
			   name="checosa" />
		<input type="hidden" 
		       value="abbandona" 
			   name="op" /> 
	 ...
	 <form action="popup.php?page=scheda_oggetti"
		...
		<input type="hidden" 
			   ...
			   name="id_oggetto" />
		<input type="submit" 
		       value="<?php echo gdrcd_filter('out',$MESSAGE['interface']['sheet']['items']['list']['put_away']);?>" />
	 </form>
...
<?php if($record['ubicabile']>ZAINO){ 
	 ...
	 if ($record['posizione']==1){
	    ...
	    <form action="popup.php?page=scheda_equip"
		   ...
		   <input type="hidden" 
			      ...
			      name="op" /> 
	       <input type="hidden" 
		          value="<?php echo $record['ubicabile']; ?>" 
		          name="posizione" />
		   <input type="hidden" 
		          value="<?php echo $record['id_oggetto']; ?>" 
			      name="id_oggetto" />
		   <input type="submit" 
		          value="<?php echo gdrcd_filter('out',$MESSAGE['interface']['sheet']['items']['list']['wear']);?>" />
	    </form>
	    <?php } //if 
	 ...
	 <form action="popup.php?page=scheda_equip"
		...
		<input type="hidden" 
			   ...
			   name="id_oggetto" />
		<input type="submit" 
		       value="<?php echo gdrcd_filter('out',$MESSAGE['interface']['sheet']['items']['list']['unwear']);?>" />
	 </form>
	...
	}else
	{
	 ...
	 <form action="popup.php?page=scheda_equip"
		...
		<input type="hidden" 
			   ...
			   name="op" />
		<input type="submit" 
		       value="<?php echo gdrcd_filter('out',$MESSAGE['interface']['sheet']['items']['list']['give']);?>" />
