  ...
  <?php while ($row=gdrcd_query($result, 'fetch')){ 
	...
	<tr>
		...
		<div class="icone_elenco">
		   <img 
		        src="themes/<?php echo gdrcd_filter('out',$PARAMETERS['themes']['current_theme']); ?>/imgs/guilds/<?php echo gdrcd_filter('out',$row['immagine']); ?>" />
		</div>
		</td>
		<td>
...
<?php /*Visualizzazione estesa gilda*/
	...
	<?php while ($row=gdrcd_query($result, 'fetch')){ ?>
    <tr>
	<td><div class="icone_elenco">
		   <?php if ($row['capo']==1){ ?><img src="imgs/icons/crown1.gif" /><?php } else {?>&nbsp;<?php } ?>
		</div></td>
	<td><div class="icone_elenco">
