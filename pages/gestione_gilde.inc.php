...
<?php /*Inserimento di un nuovo ruolo nella gilda corrente*/
      if (gdrcd_filter('get',$_POST['op'])=='nuovo_ruolo') { 
	       ...
	       if ($_POST['immagine']==''){$immagine="standard_gilda.png";}
...
<?php /* Cancellatura in un ruolo */
      if ((gdrcd_filter('get',$_POST['op'])==$MESSAGE['interface']['administration']['guilds']['role']['submit']['delete']) && 
	      ($_POST['provenienza']=='ruolo')) { /*Eseguo la cancellatura*/
...
<?php /*Modifica di un ruolo*/
	  if ((gdrcd_filter('get',$_POST['op'])==$MESSAGE['interface']['administration']['guilds']['role']['submit']['edit']) &&
	     ($_POST['provenienza']=='ruolo')){  
		 /*Processo le informazioni ricevute dal form*/
         if ((isset($_POST['capo'])==TRUE)&&($_POST['capo']=='is_capo')){$is_capo=1;}
         else {$is_capo=0;}
	     if ($_POST['immagine']==''){$immagine="standard_ruolo.png";}
		 else{$immagine=gdrcd_filter('in',$_POST['immagine']);}
         /*Eseguo l'aggiornamento*/
	...
	if ((gdrcd_filter('get',$_REQUEST['op'])=='edit') || 
	  ...
	  if((isset($_REQUEST['id_record'])===FALSE)||(gdrcd_filter('get',$_REQUEST['id_record']>-1))){ ?>
	    ...
	    <form action="main.php?page=gestione_gilde"
		  ...
		  <div class='form_field'>
	         <input name="url_sito" 
				    value="<?php if(isset($loaded_record['url_sito'])===TRUE){echo gdrcd_filter('out',$loaded_record['url_sito']);}
		                         else {echo "http://";} ?>" />
		  </div>
...
<!-- Titolo della pagina -->
    ...
    <div class="page_body">
       ...
       <form action="main.php?page=gestione_gilde"
          ...
          <input type="hidden"
		         name="gilda"
				 value="<?php echo gdrcd_filter('out',$id_gilda_padre); ?>" /> 
		  <input type="hidden"
		         name="op"
				 value="nuovo_ruolo" /> 
		  <input type="submit" 
    ...
    <form action="main.php?page=gestione_gilde"
		  ...
		  <div class='form_field'>
	         <input name="stipendio" 
				    value="<?php echo 0+gdrcd_filter('out',$row['stipendio']);?>" />
		  </div>
		  </tr>
          ...
          <input type="hidden"
		         name="id_ruolo"
				 value="<?php echo gdrcd_filter('out',$row['id_ruolo']); ?>" /> 
		  <input type="hidden"
		         name="gilda"
				 value="<?php echo gdrcd_filter('out',$id_gilda_padre); ?>" /> 
          <input type="submit" 
	...
	if ($numresults>0){ ?>
       ...
       <table>
          ...
          <tr>
		     ...
		     <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['guilds']['type']); ?></div></td>
			 <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['guilds']['visible']); ?></div></td>
	      ...
	      <tr>
		     ...
		     <td class="casella_controlli"><!-- Iconcine dei controlli -->
				...
				<div class="controllo_elenco" >
		           <form class="opzioni_elenco_record_gestione" action="main.php?page=gestione_gilde" method="post">
			          <input type="hidden" name="id_record" value="<?php echo gdrcd_filter('out',$row['id_gilda'])?>" />
