...
<?php /*Inserimento di un nuovo record*/
    if (gdrcd_filter('get',$_POST['op'])=='insert') {  
       ...
       if ((isset($_POST['mobile'])==TRUE)&&($_POST['mobile']=='is_mobile')){$is_mobile=1;}
       else {$is_mobile=0;}
	   if ($_POST['immagine']==""){$immagine="standard_mappa.png";}
...
<?php /*Modifica di un record*/
	if (gdrcd_filter('get',$_POST['op'])=='modify'){ 
       ...
       if ((isset($_POST['mobile'])==TRUE) &&
		   ($_POST['mobile']=='is_mobile')){$is_mobile=1;}
       else {$is_mobile=0;}
	   if ($_POST['immagine']==""){$immagine="standard_mappa.png";}
	   else{$immagine=gdrcd_filter('in',$_POST['immagine']);}
...
<?php /*Form di inserimento/modifica*/
    ...
    <form action="main.php?page=gestione_mappe"
	      method="post"
		  class="form_gestione">
          ...
          <div class='form_field'>
	         <input name="nome" 
			        value="<?php echo gdrcd_filter('out',$loaded_record['nome']); ?>" />
		  </div>
		  ...
		  <div class='form_field'>
	         <input name="posizione" 
		            value="<?php echo 0+gdrcd_filter('out',$loaded_record['posizione']); ?>" />
		  </div>
		  <div class='form_info'>
...
-->	
		  ...
		  <div class='form_field'>
	         <input name="larghezza" 
				    value="<?php echo gdrcd_filter('out',$loaded_record['larghezza']); ?>" />
		  </div>
		   <div class='form_info'>
		  ...
		  <div class='form_field'>
	         <input name="altezza" 
				    value="<?php echo gdrcd_filter('out',$loaded_record['altezza']); ?>" />
		  </div>
		  <div class='form_info'>
		  ...
		  <div class='form_submit'>  
			  ...
			  <input type="submit" 
			         value="<?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['maps']['submit']['edit']);?>" />
			  <input type="hidden" name="op" value="modify">
			  <?php	} /* Altrimenti il tasto inserisci */
					  else { ?>
			  <input type="hidden" name="op" value="insert">
			  <input type="submit" 
			         value="<?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['maps']['submit']['insert']);?>" />
			  <?php	} ?>
	...
	if ($numresults>0){ ?>
       ...
       <table>
          ...
          <tr>
		     ...
		     <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['maps']['position']); ?></div></td>
			 <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['maps']['is_mobile']); ?></div></td>
	      ...
	      <tr class="risultati_elenco_record_gestione">
		     ...
		     <td class="casella_elemento"><div class="elementi_elenco"><?php echo gdrcd_filter('out',$row['posizione']); ?></div></td>
			 <td class="casella_elemento"><div class="elementi_elenco"><?php if ($row['mobile']==1){ echo gdrcd_filter('out',$MESSAGE['interface']['administration']['yes']); }
