...
if (isset($_REQUEST['pg'])===FALSE){
   ...
   if ($_POST['op']=='modify'){
      gdrcd_query("UPDATE personaggio SET email = '".gdrcd_filter('in',$_POST['modifica_email'])."', affetti = '".gdrcd_filter('in',$_POST['modifica_affetti'])."', descrfisica = '".gdrcd_filter('in',$_POST['modifica_descrfisica'])."', descrizione = '".gdrcd_filter('in',$_POST['modifica_background'])."', url_media = '".gdrcd_filter('in',$_POST['modifica_url_media'])."', url_img = '".gdrcd_filter('in',$_POST['modifica_url_img'])."', car0 = ".gdrcd_filter('num',$_POST['car0']).", car1 = ".gdrcd_filter('num',$_POST['car1']).", car2 = ".gdrcd_filter('num',$_POST['car2']).", car3 = ".gdrcd_filter('num',$_POST['car3']).", car4 = ".gdrcd_filter('num',$_POST['car4']).",  car5 = ".gdrcd_filter('num',$_POST['car5']).", sesso = '".gdrcd_filter('in',$_POST['modifica_sesso'])."', id_razza = ".gdrcd_filter('num',$_POST['modifica_razza']).", id_nazione = ".gdrcd_filter('num',$_POST['modifica_nazione']).", banca=".gdrcd_filter('num',$_POST['modifica_banca']).", salute_max=".gdrcd_filter('num',$_POST['modifica_salute_max'])." WHERE nome = '".gdrcd_filter('in',$_REQUEST['pg'])."' AND permessi <= ".$_SESSION['permessi']."");
		   echo '<div class="warning">'.gdrcd_filter('out',$MESSAGE['warning']['modified']).'</div>';
   } else {
	   /*Carico le informazioni del PG*/
	   
	   $record = gdrcd_query("SELECT email, sesso, id_razza, id_nazione, descrizione, descrfisica, affetti, url_img, url_media, car0, car1, car2, car3, car4, car5, salute_max, banca  FROM personaggio WHERE nome='".gdrcd_filter('in',$_REQUEST['pg'])."'");
   }
?>
...
?> 
  ...
  <form action="main.php?page=scheda_gst" method="post">
	...
	<div class='form_label'>
	   <?php echo gdrcd_filter('out',$MESSAGE['interface']['sheet']['modify_form']['admin']['descrfisica']); ?>
	</div>
	<div class='form_field'>
