...
$query="SELECT  descrizione_evento, data_evento  FROM log WHERE nome_interessato = '".gdrcd_filter('in',$_REQUEST['pg'])."'  AND codice_evento = ".ACCOUNTMULTIPLO." ORDER BY data_evento DESC LIMIT ".$num_logs."";
$result=gdrcd_query($query, 'result');
...
$query="SELECT  destinatario, spedito, testo  FROM messaggi WHERE mittente = '".gdrcd_filter('in',$_REQUEST['pg'])."' ORDER BY spedito DESC LIMIT ".$num_logs."";
$result=gdrcd_query($query, 'result');
...
<tr>
  ...
  <td class="casella_titolo">
     <div class="titoli_elenco">
	    <?php echo gdrcd_filter('out',$MESSAGE['interface']['sheet']['log']['message']); ?>
	 </div>
  </td>
</tr>
<?php while ($record=gdrcd_query($result, 'fetch')){?>
...
<?php /*Seleziono gli ultimi login*/
$query="SELECT  descrizione_evento, data_evento, autore  FROM log WHERE nome_interessato = '".gdrcd_filter('in',$_REQUEST['pg'])."'  AND codice_evento = ".CHANGEDNAME." ORDER BY data_evento DESC LIMIT ".$num_logs."";
$result=gdrcd_query($query, 'result');
if (gdrcd_query($result, 'num_rows')>0){
?>
<!-- Intestazione tabella elenco -->
