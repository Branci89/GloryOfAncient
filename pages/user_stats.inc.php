...
<div class="page_title"><h2><?php echo gdrcd_filter('out',$MESSAGE['interface']['user']['stats']['page_name']); ?></h2></div>
...
<div class="page_body">
		...
		<table>
		<?php
		$row=gdrcd_query("SELECT MIN(data_iscrizione) AS stat FROM personaggio");?>
		<tr>
        ...
        <?php
		$row=gdrcd_query("SELECT COUNT(*) AS stat FROM personaggio");?>
		<tr>
		<td class="casella_elemento"><div class="elementi_elenco">
		...
		<?php
		$row=gdrcd_query("SELECT COUNT(*) AS stat FROM personaggio WHERE esilio > NOW()");?>
		<tr>
		<td class="casella_elemento"><div class="elementi_elenco">
		...
		<?php
		$row=gdrcd_query("SELECT COUNT(*) AS stat FROM personaggio WHERE permessi = ".GAMEMASTER."");?>
		<tr>
		<td class="casella_elemento"><div class="elementi_elenco">
		...
		<?php
		$row=gdrcd_query("SELECT COUNT(*) AS stat FROM personaggio WHERE permessi >= ".MODERATOR."");?>
		<tr>
		<td class="casella_elemento"><div class="elementi_elenco">
        ...
        <?php
		$row=gdrcd_query("SELECT COUNT(*) AS stat FROM messaggioaraldo WHERE data_messaggio > DATE_SUB(NOW(), INTERVAL 7 DAY)");?>
		<tr>
		<td class="casella_elemento"><div class="elementi_elenco">
		...
		<?php if(gdrcd_filter_get($_REQUEST['op'])=='esiliati'){/*Esiliati*/ 
		...
		<tr>
		<td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['user']['stats']['character']); ?></div></td>
		<td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['user']['stats']['date_end']); ?></div></td>
		<td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['user']['stats']['why']); ?></div></td>
	    ...
	    <?php while($row=gdrcd_query($result, 'fetch')){ ?>
			  ...
			  <td class="casella_elemento"><div class="elementi_elenco"><?php echo gdrcd_format_date($row['esilio']); ?></div></td>
			  <td class="casella_elemento"><div class="elementi_elenco"><?php echo gdrcd_filter('out',$row['motivo_esilio']).' ('.gdrcd_filter('out',$row['autore_esilio'].', '.gdrcd_format_date($row['data_esilio']).') '); ?></div></td>
