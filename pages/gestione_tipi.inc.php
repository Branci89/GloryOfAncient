...
<!-- Titolo della pagina -->
<?php if (isset($_REQUEST['types'])===FALSE){$types='items';}
      else{$types=gdrcd_filter('get',$_REQUEST['types']); }?>
...
<?php /*Inserimento di un nuovo record*/
    if ($_POST['op']=='insert') {  
	   ...
	   if($types=="items"){$query="INSERT INTO codtipooggetto (descrizione) VALUES ('".gdrcd_filter('in',$_POST['nome'])."')";}
	   if($types=="guilds"){$query="INSERT INTO codtipogilda (descrizione) VALUES ('".gdrcd_filter('in',$_POST['nome'])."')";}
	   if($types=="talents"){$query="INSERT INTO codtipotalento (descrizione) VALUES ('".gdrcd_filter('in',$_POST['nome'])."')";}
       gdrcd_query($query);?>
	   <div class="warning">  
...
<?php /*Cancellatura in un record*/
    if ($_POST['op']=='erase'){ 
	   ...
	   if($types=="items"){$query="DELETE FROM codtipooggetto WHERE cod_tipo=".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1";
	       $query_update="UPDATE oggetto SET tipo = 0 WHERE tipo = ".gdrcd_filter('num',$_POST['id_record'])."";
	   }
       if($types=="guilds"){$query="DELETE FROM codtipogilda WHERE cod_tipo=".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1";
	       $query_update="UPDATE gilda SET tipo = 0 WHERE tipo = ".gdrcd_filter('num',$_POST['id_record'])."";
	   }
if($types=="talents"){$query="DELETE FROM codtipotalento WHERE cod_tipo=".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1";
	       $query_update="UPDATE talento SET tipo = 0 WHERE tipo = ".gdrcd_filter('num',$_POST['id_record'])."";
	   }
       gdrcd_query($query);
...
<?php /*Modifica di un record*/
	if ($_POST['op']=='modify'){ 
	   ...
	   if($types=="items"){$query="UPDATE codtipooggetto SET descrizione ='".gdrcd_filter('in',$_POST['nome'])."' WHERE cod_tipo = ".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1";}
	   if($types=="guilds"){$query="UPDATE codtipogilda SET descrizione ='".gdrcd_filter('in',$_POST['nome'])."' WHERE cod_tipo = ".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1";}
	    if($types=="talents"){$query="UPDATE codtipotalento SET descrizione ='".gdrcd_filter('in',$_POST['nome'])."' WHERE cod_tipo = ".gdrcd_filter('num',$_POST['id_record'])." LIMIT 1";}
	   gdrcd_query($query); ?>	   
	   <div class="warning">
...
<?php /*Form di inserimento/modifica*/
	if ((gdrcd_filter('get',$_POST['op'])=='edit') || (gdrcd_filter('get',$_REQUEST['op'])=='new')){ 
	  ...
	  if (gdrcd_filter('get',$_POST['op'])=='edit'){
		 ...
		 if($types=="items"){ $query="SELECT * FROM codtipooggetto WHERE cod_tipo=".gdrcd_filter('in',$_POST['id_record'])." LIMIT 1 ";}
		 if($types=="guilds"){ $query="SELECT * FROM codtipogilda WHERE cod_tipo=".gdrcd_filter('in',$_POST['id_record'])." LIMIT 1 ";}
		  if($types=="talents"){ $query="SELECT * FROM codtipotalento WHERE cod_tipo=".gdrcd_filter('in',$_POST['id_record'])." LIMIT 1 ";}
    ...
    <form action="main.php?page=gestione_tipi&types=<?php echo $types; ?>"
		  ...
		  <div class='form_submit'>  
			  <?php /* Se l'operazione è una modifica stampo i tasti modifica e annulla */
			        if ($operation == "edit"){?>
			  <input type="hidden" name="id_record" value="<?php echo $loaded_record['cod_tipo'];?>" />
			  <input type="submit" 
...
<?php if ((isset($_POST['op'])===FALSE)&&(isset($_REQUEST['op'])===FALSE)) { 
	...
	if ($types=='items'){$query="SELECT COUNT(*) FROM codtipooggetto";}
	if ($types=='guilds'){$query="SELECT COUNT(*) FROM codtipogilda";}
	if ($types=='talents'){$query="SELECT COUNT(*) FROM codtipotalento";}
	$record_globale=gdrcd_query($query);
	$totaleresults=$record_globale['COUNT(*)'];
	//Lettura record
	if ($types=='items'){$query= "SELECT cod_tipo, descrizione FROM codtipooggetto ORDER BY descrizione LIMIT ".$pagebegin.", ".$pageend."";}
	if ($types=='guilds'){$query= "SELECT cod_tipo, descrizione FROM codtipogilda ORDER BY descrizione LIMIT ".$pagebegin.", ".$pageend."";}
	if ($types=='talents'){$query= "SELECT cod_tipo, descrizione FROM codtipotalento ORDER BY descrizione LIMIT ".$pagebegin.", ".$pageend."";}
	...
	if ($numresults>0){ ?>
       ...
       <table>
          ...
          <tr>
		     <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['types']['name']);?></div></td>
			 <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['administration']['types']['code']); ?></div></td>
	      ...
	      <tr class="risultati_elenco_record_gestione">
             <td class="casella_elemento"><div class="elementi_elenco"><?php echo gdrcd_filter('out',$row['descrizione']); ?></div></td>
		     <td class="casella_elemento"><div class="elementi_elenco"><?php echo gdrcd_filter('out',$$row['cod_tipo']); ?></div></td>
			 <td class="casella_controlli"><!-- Iconcine dei controlli -->
		        <!-- Modifica -->
				<div class="controlli_elenco">
				<div class="controllo_elenco" >
		           <form class="opzioni_elenco_record_gestione" action="main.php?page=gestione_tipi&types=<?php echo $types; ?>" method="post">
			          <input type="hidden" name="id_record" value="<?php echo $row['cod_tipo']?>" />
	 ...
	 <?php } ?>
     <?php if ($types=='talents'){ ?>
	 <div class="link_back">
        <a href="main.php?page=gestione_talenti">
	      Gestione Doni
		</a>
     </div>
	 <?php } ?>
