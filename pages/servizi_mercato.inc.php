		 ...
		 <tr>
                     <td  class="casella_elemento">
                        <div class="elementi_elenco">
                            <a href="main.php?page=servizi_mercato&op=visit&what=<?php echo gdrcd_filter('out', $row['cod_tipo']); ?>"><?php echo gdrcd_bbcoder(gdrcd_filter('out', $row['descrizione'])); ?></a>
                        </div>
                     </td>
		 </tr> 
		 <?php }//while 
	...
	if ($numresults>0){ ?>
       ...
       <table>
          ...
          <tr>
		     ...
		     <td class="casella_titolo"><div class="titoli_elenco"><?php echo gdrcd_filter('out',$MESSAGE['interface']['market']['price']); ?></div></td>
		  </tr>
		  <!-- Record -->
          <?php while ($row=gdrcd_query($result, 'fetch')){ ?>
	      <tr>
		...
		*/	
		if ($PARAMETERS['mode']['user_bbcode'] == 'ON')
		{
			...
			{
				echo bbdecoder(gdrcd_html_filter($row['descrizione']), true);
			
			}elseif ($PARAMETERS['settings']['user_bbcode']['type'] == 'bbd')
			{
				echo bbdecoder(gdrcd_filter('out',$row['descrizione']), true);
		...
		{
			echo gdrcd_html_filter($row['descrizione']);
		}
