...
if ((empty($record) === FALSE) && ($record['permessi'] > -1) && (strtotime($record['ora_entrata']) < strtotime($record['ora_uscita']) || (strtotime($record['ultimo_refresh']) + 300) < time())) {
    ...
    //Per identificare chi lavora al mercato e dargli accesso alla gestione:
    $queryGuild = "SELECT COUNT(personaggio) AS presente FROM clgpersonaggioruolo as pgRole LEFT JOIN ruolo as R ON pgRole.id_ruolo = R.id_ruolo WHERE pgRole.personaggio = '" . $_SESSION['login'] . "' AND R.gilda = 10";
    $resGuild = gdrcd_query($queryGuild, 'query');
    $flagPresente = (int) $resGuild['presente'] > 0 ? true : false;
